...
{
	string res = message;
	size_t remaining = 1088 - ( ( message.size() * 8 ) % 1088 );
	if ( remaining )
	{
		if ( remaining == 8 )
		{
	...
	assert( res.size() && res.size() % 136 == 0 );
	return res;
}
...
string SHA3_512( string message )
{
	...
	unsigned long long message_block_count = ( message.size() * 8 ) / 1088;
...
int main()
{
	...
	cout << "SHA3_256:"<<binary_to_HEX( digest ) << endl;
